...
impl<'a, K, V> NodeRef<'a, K, V> {
    ...
    unsafe fn from_shared(shared: Shared<'a, Node<K, V>>) -> Option<Self> {
        if let Some(ptr) = NonNull::new(shared.as_raw().cast_mut()) {
    ...
    fn as_tower(self) -> TowerRef<'a, K, V> {
        ...
        unsafe {
            TowerRef::new(NonNull::new_unchecked(
                ptr::addr_of!((*self.ptr.as_ptr()).tower).cast_mut(),
            ))
        }
    }

    /// Gwts a plain reference to the node which can be used for anything that doesn't need to acces
    /// the tower.
    #[inline]
    ...
    K: Ord,
{
    ...
    unsafe fn help_unlink<'a>(
        ...
        match pred.compare_exchange(
            Shared::from(curr.ptr.as_ptr().cast_const()),
            succ.with_tag(0),
            Ordering::Release,
            Ordering::Relaxed,
            guard,
            ...
            loop {
                ...
                // We failed. Let's search for the key and try again.
                {
                    ...
                    impl<K, V> Drop for ScopeGuard<K, V> {
                        fn drop(&mut self) {
                            unsafe { Node::finalize(self.0.cast_mut()) }
                        }
                    }
                ...
                if let Some(r) = search.found {
                    ...
                    if replace {
                        ...
                        if let Some(e) = RefEntry::try_acquire(self, r) {
                            ...
                            Node::finalize(node.as_raw().cast_mut());
                            self.hot_data.len.fetch_sub(1, Ordering::Relaxed);
                ...
                if n.mark_tower() {
                    ...
                    for level in (0..n.height()).rev() {
                        ...
                        if search.left[level]
                            .get_level(level)
                            .compare_exchange(
                                Shared::from(n.ptr.as_ptr().cast_const()),
                                succ,
                                Ordering::SeqCst,
                                Ordering::SeqCst,
                                guard,
    ...
    fn next(&mut self) -> Option<Entry<'a, 'g, K, V>> {
        ...
        if let Some(h) = self.head {
            let bound = match self.tail {
                Some(t) => Bound::Excluded(t.as_ref().key.borrow()),
    ...
    fn next_back(&mut self) -> Option<Entry<'a, 'g, K, V>> {
        ...
        if let Some(t) = self.tail {
            let bound = match self.head {
                Some(h) => Bound::Excluded(h.as_ref().key.borrow()),
